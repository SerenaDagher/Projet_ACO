...
int main(int nargs, char* argv[])
{
    ...
    int max_iterations = -1; // -1 = exécution jusqu'à fermeture de la fenêtre
    int render_freq = 10; // Afficher 1 frame sur N (10 par defaut)
    
    for (int i = 1; i < nargs; ++i) {
        std::string arg = argv[i];
        if (arg == "--benchmark") {
            benchmark_mode = true;
            max_iterations = 5000; // Itérations par défaut en mode benchmark
    ...
    while (cont_loop) {
        ...
        if (!benchmark_mode && (it % render_freq == 0)) {
            auto t1 = std::chrono::high_resolution_clock::now();
