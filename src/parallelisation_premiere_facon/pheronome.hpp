...
public:
    ...
    void evaporate_rows(std::size_t row_begin, std::size_t row_end) {
        for (std::size_t i = row_begin; i < row_end; ++i) {
            for (std::size_t j = 1; j <= m_dim; ++j) {
                m_buffer_pheronome[(i + 1) * m_stride + j][0] *= m_beta;
                m_buffer_pheronome[(i + 1) * m_stride + j][1] *= m_beta;
            }
        }
    }
