...
public:
    ...
    std::size_t raw_buffer_count() const {
        return m_buffer_pheronome.size() * 2;
    }

    double* raw_buffer_data() {
        return reinterpret_cast<double*>(m_buffer_pheronome.data());
    }

    const double* raw_buffer_data() const {
        return reinterpret_cast<const double*>(m_buffer_pheronome.data());
    }

    void evaporate_rows_parallel(std::size_t row_begin, std::size_t row_end) {
#ifdef _OPENMP
#pragma omp parallel for schedule(static)
#endif
        for (long i = static_cast<long>(row_begin); i < static_cast<long>(row_end); ++i) {
            for (std::size_t j = 1; j <= m_dim; ++j) {
                m_buffer_pheronome[(static_cast<std::size_t>(i) + 1) * m_stride + j][0] *= m_beta;
                m_buffer_pheronome[(static_cast<std::size_t>(i) + 1) * m_stride + j][1] *= m_beta;
            }
        }
    }
