...
struct TimingStats {
    ...
    void print() const {
        ...
        std::cout << "\n============================================================" << std::endl;
        std::cout << "     STATISTIQUES DE PERFORMANCE - MODE VECTORISE" << std::endl;
        std::cout << "============================================================" << std::endl;
                   ...
                   AntColony& colony, std::size_t& cpteur, TimingStats& stats )
{
    ...
    for ( std::size_t i = 0; i < colony.size(); ++i )
        advance_ant( i, colony, phen, land, pos_food, pos_nest, cpteur );
...
int main(int nargs, char* argv[])
{
    ...
    std::cout << "  Representation: Vectorisee (SoA)" << std::endl;
    std::cout << "  Mode: " << (benchmark_mode ? "Benchmark" : "Interactif") << std::endl;
    if (max_iterations > 0)
        std::cout << "  Iterations max: " << max_iterations << std::endl;
    if (!benchmark_mode)
        std::cout << "  Frequence rendu: 1 frame tous les " << render_freq << " iterations" << std::endl;
    std::cout << std::endl;
    
    // Génération du territoire
    ...
    // Initialisation de la colonie vectorisée (SoA)
    AntColony colony;
    colony.x.resize(nb_ants);
    colony.y.resize(nb_ants);
    colony.states.resize(nb_ants, 0);  // toutes non chargées
    ...
    // Pas de renderer en mode vectorisé (benchmark uniquement)
    // Le renderer original attend un std::vector<ant>, incompatible avec AntColony.
    // Pour visualiser, utiliser ant_simu (version originale).
    Window* win = nullptr;
    if (!benchmark_mode) {
